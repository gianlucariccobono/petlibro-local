...
The scheduler will use the configured local timezone and date-aware rollover, checking shortly after midnight so a daily action advances to the correct local date. Each Polar can hold only three plans. Because the MQTT plan objects are immutable, the scheduler will materialize a new plan object for each day rather than edit an existing object. Global schedule reconciliation replaces the entire plan list on every Polar: it clears existing or legacy device plans and sends only schedules created in the **Schedules** menu. Those existing plans are removed on the next successful sync. If a Polar is offline, reconciliation and newly materialized plans cannot be synchronized until it reconnects, so its current plan list remains until then and delivery may be delayed.

> **Polar schedule sync:** Plans created directly on the device or in the Petlibro app are not preserved by global reconciliation. They are removed when that Polar next syncs successfully, and only schedules from the **Schedules** menu are sent afterward.
